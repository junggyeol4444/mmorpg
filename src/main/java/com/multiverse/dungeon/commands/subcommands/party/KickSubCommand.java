...
public class KickSubCommand {
     ...
     * 생성자
     */
    public KickSubCommand(DungeonCore plugin) {
        this. plugin = plugin;
    }
    ...
    public void execute(Player player, String[] args) {
        if (args.length < 1) {
            MessageUtils.sendError(player, "사용법: /party kick <플레이어명>");
            return;
        }
        ...
        if (!  party.isLeader(player.  getUniqueId())) {
            MessageUtils.sendError(player, "파티 리더만 추방할 수 있습니다.");
            return;
        }
        ...
        if (party.isLeader(target.getUniqueId())) {
            MessageUtils.sendError(player, "리더는 추방할 수 없습니다.");
            return;
        }

        plugin.getPartyManager().kickPlayer(party. getPartyId(), target. getUniqueId());
        
        MessageUtils.sendSuccess(player, target.getName() + "을(를) 파티에서 추방했습니다.");
        ...
        for (var memberId : party.getMembers()) {
            ...
            if (member != null && member.isOnline()) {
                member.sendMessage("§c[파티] " + target. getName() + "이(가) 추방되었습니다.");
            }
        }
    }
}