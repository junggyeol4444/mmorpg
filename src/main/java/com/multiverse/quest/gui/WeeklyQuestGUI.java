...
/**
 * 주간 퀨스트 GUI
 * 플레이어가 주간 퀨스트를 조회하고 수락합니다.
 */
public class WeeklyQuestGUI {
     ...
     * @param player 플레이어
     */
    public WeeklyQuestGUI(QuestDataManager questDataManager, Player player) {
     ...
     */
    public void open() {
        inventory = Bukkit.createInventory(null, INVENTORY_SIZE, "§6【 주간 퀨스트 】");
     ...
     * 주간 퀨스트 표시
     */
    private void displayWeeklyQuests() {
        ...
        for (Quest quest : weeklyQuests) {
            ...
            boolean canAccept = questDataManager.canAcceptWeeklyQuest(playerUUID, quest. getQuestId());
            ItemStack questItem = createWeeklyQuestItem(quest, canAccept);
    ...
    private ItemStack createTitleItem() {
        ...
        if (meta != null) {
            meta.setDisplayName("§f주간 퀨스트");

            List<String> lore = new ArrayList<>();
            lore.add("§7매주 월요일에 초기화됩니다.");
     ...
     * 주간 퀨스트 아이템 생성
     */
    private ItemStack createWeeklyQuestItem(Quest quest, boolean canAccept) {
        ...
        if (meta != null) {
            ...
            if (canAccept) {
                ...
                lore.add("§7이미 이번 주에 완료했습니다.");
                lore.add("§7다음 주에 다시 시도하세요.");
            }
    ...
    private ItemStack createStatisticsItem() {
        ...
        if (meta != null) {
            ...
            List<String> lore = new ArrayList<>();
            List<Quest> weeklyQuests = questDataManager. getQuestManager().getQuestsByType(QuestType. WEEKLY);
            int totalWeekly = weeklyQuests.size();
            int completedThisWeek = (int) weeklyQuests.stream()
                .filter(q -> ! questDataManager.canAcceptWeeklyQuest(playerUUID, q.getQuestId()))
                .count();

            lore.add("§7주간 퀨스트: §f" + totalWeekly);
            lore.add("§7이번 주 완료: §a" + completedThisWeek);
            lore.add("§7남은 퀨스트: §e" + (totalWeekly - completedThisWeek));
    ...
    private ItemStack createNoQuestItem() {
        ...
        if (meta != null) {
            meta.setDisplayName("§c주간 퀨스트 없음");

            List<String> lore = new ArrayList<>();
            lore.add("§7사용 가능한 주간 퀨스트가 없습니다.");
